    ...
    for p_val in (2, 4):
        ...
        specs.append(
            RunSpec(
                ...
                tags={"p": str(p_val), "m": "256", "n": "1024", "k": "lt_n", "kind": "gram"},
            )
        )
...
def _filter_specs(
    ...
    if only_tokens:
        ...
        out = [s for s in out if any(t in f"{s.name} {s.txt_out}".lower() for t in toks)]
...
def _sanitize_args(args: argparse.Namespace, repo_root: str) -> dict[str, Any]:
    ...
    path_keys = [
        "out", "ab_out", "manifest_out", "ab_baseline_rows_in", 
        "baseline_rows_out", "best_dir", "baseline_dir", "golden_dir"
    ]
    for k in path_keys:
        if safe_args.get(k):
...
def main() -> None:
    ...
    parser.add_argument("--kinds", type=str, default="", help="Comma-separated kinds (spd,nonspd,gram)")
    parser.add_argument("--p-vals", type=str, default="", help="Comma-separated p values (1,2,4)")
    parser.add_argument("--sizes", type=str, default="", help="Comma-separated sizes (256,512,1024)")
    parser.add_argument("--methods", type=str, default="", help="Comma-separated methods to run.")
    ...
    parser.add_argument(
        "--ab-baseline-latest", action="store_true", help="Use latest run manifest as baseline."
    )
    parser.add_argument(
    ...
    parser.add_argument("--update-baseline", action="store_true", help="Update baseline results with current run.")
    parser.add_argument("--baseline-dir", type=str, default="benchmark_results/baseline")
    parser.add_argument("--from-baseline", action="store_true", help="Generate report from baseline results instead of running benchmarks.")
    ...
    if args.baseline and not args.ab_baseline_rows_in:
        baseline_path = os.path.join(REPO_ROOT, args.baseline_dir, "baseline_solver.json")
    ...
    if args.ab_baseline_latest and not args.ab_baseline_rows_in:
        import glob
        all_manifests = glob.glob(str(REPO_ROOT / "benchmark_results/runs/**/*run_manifest.json"), recursive=True)
        if all_manifests:
    ...
    if args.from_baseline:
        ...
        if not os.path.exists(baseline_path):
            raise FileNotFoundError(f"Cannot use --from-baseline: {baseline_path} not found.")
        ...
        print(f"Done (from baseline). Report at: {repo_relative(args.out, str(REPO_ROOT))}")
        return
    ...
    if ab_mode:
        ...
        if args.update_baseline:
            baseline_path = os.path.join(REPO_ROOT, args.baseline_dir, "baseline_solver.json")
            print(f"[baseline] Promoting current run to baseline results at {baseline_path}")
    ...
    if args.baseline_rows_out:
        write_json_file(args.baseline_rows_out, {"rows": [row_to_dict(r) for r in (rows_a if ab_mode else all_rows)]})
